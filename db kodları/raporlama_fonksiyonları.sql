...
--Bu fonksiyonun amacı içerik üreticisinin temel bilgilerini görmektir.
CREATE OR REPLACE FUNCTION
	fn_get_creator_stats(
		p_content_creator_id UUID
	)
RETURNS TABLE (
	creator_name TEXT,	--İÇerik üreticisinin adı
	total_contents BIGINT,	--İçerik üreticisinin içerik sayısı
    average_rating DECIMAL(5,2),	--Ortalama aldığı rating
    total_comments BIGINT,	--İçeriklerine kaç yorum atılmış
    total_followers BIGINT,	--Takipçi sayısı
    total_revenue DECIMAL(15,2)	--Kazanılan para
)
AS $$
BEGIN
	RETURN QUERY
	SELECT
		...
		(
			...
			FROM
				follow_relations
			WHERE
				content_creator_id = p_content_creator_id
		),
